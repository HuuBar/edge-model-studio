...
from modeling_mole_rep_fp16 import MoleForCausalLM
from transformers import TextIteratorStreamer
import threading

def clear_os_page_cache():
    """清空 Linux 文件系统缓存（需要 root 权限）"""
    try:
        subprocess.run(["sync"], check=True)
        subprocess.run(["sudo", "sh", "-c", "echo 3 > /proc/sys/vm/drop_caches"], check=True)
        print("[Cache] 已清理系统页缓存")
    except Exception as e:
        print(f"[Cache] 清理页缓存失败: {e}")
...
def get_cpu_memory():
    ...
    try:
        ...
        print(f"CPU内存统计失败: {str(e)}")
        return 0.0

def get_gpu_memory():
    """使用nvidia-smi获取当前进程显存占用(MB)"""
    try:
        pid = os.getpid()
...
def write_io_point(label, io_file):
    ...
    with open(io_file, "a") as f:
        ...
        f.write(f"{label}_read_chars,{io_vals.get('rchar',0)}\n")
        f.write(f"{label}_write_chars,{io_vals.get('wchar',0)}\n")
...
def calculate_io_delta(start_io, end_io):
    ...
    return {
        ...
        "read_chars": (end_io.get('rchar',0) - start_io.get('rchar',0)) / (1024*1024),
        "write_chars": (end_io.get('wchar',0) - start_io.get('wchar',0)) / (1024*1024)
    }
...
model = MoleForCausalLM.from_pretrained(MODEL_PATH, device_map='cuda', torch_dtype=torch.float16, local_files_only=True)


# 记录加载后IO状态
load_end_io = read_proc_io()
write_io_point("load_end", IO_FILE)
load_io_delta = calculate_io_delta(load_start_io, load_end_io)    

# ROM: lut_path 修正
lut_path = model.config.lut_path
if not os.path.isabs(lut_path):
    lut_path = os.path.join(MODEL_PATH, lut_path)
model.config.lut_path = lut_path
model.model.lut_path = lut_path
...
# 记录推理前IO状态
infer_start_io = read_proc_io()
write_io_point("infer_start", IO_FILE)

for idx, prompt in enumerate(tqdm(prompts[0:1], desc="推理进度")):
