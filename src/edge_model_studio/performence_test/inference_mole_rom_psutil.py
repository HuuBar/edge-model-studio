...
def write_io_point(label, io_file):
    ...
    with open(io_file, "a") as f:
        ...
        f.write(f"{label}_read_count,{io_vals.get('read_count',0)}\n")
        f.write(f"{label}_write_count,{io_vals.get('write_count',0)}\n")
...
def calculate_io_delta(start_io, end_io):
    ...
    return {
        ...
        "read_count": end_io.get('read_count',0) - start_io.get('read_count',0),
        "write_count": end_io.get('write_count',0) - start_io.get('write_count',0),
    }
...
for idx, prompt in enumerate(tqdm(prompts[10:20], desc="推理进度")):
